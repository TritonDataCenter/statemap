...
var statebarGetTextClick = function (statebar)
{
	return (function (evt) {
		if (evt.shiftKey)
...
var statebarCreate = function (elem)
{
	...
	text.appendChild(g_svgDoc.createTextNode(globals.entityKind + ' ' +
	    elem.parentNode.id.substr(globals.entityPrefix.length)));
	...
	text.addEventListener('click', statebarGetTextClick(statebar));
...
var timebarSetTextLocation = function (text, mapX)
{
	...
	if (globals.start) {
		var s = globals.start[0] +
		    (base + offs + globals.start[1]) / 1000000000;
...
var timebarGetTextClick = function (timebar)
{
	return (function (evt) {
		if (evt.shiftKey)
...
var timebarCreate = function (mapX)
{
	...
	text.addEventListener('click', timebarGetTextClick(timebar));
	parent.appendChild(text);
